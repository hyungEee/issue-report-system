...
## 설계 의도
본 시스템은 리포트 작성 단계에서만 부분적으로 AI를 활용하였고, 데이터 수집 및 분석 과정은 명시적인 규칙 기반 설계로 구성하였습니다.
국가/카테고리별 기사 수집 비율과 중요도 산출 방식 등을 수치로 정의하여 결과의 예측 가능성과 일관성을 확보하고, 할루시네이션 리스크를 구조적으로 줄이고자 했습니다.
언론 시스템은 단순히 이슈를 나열하는 것이 아니라, 무엇을 선택하고 어떻게 보여줄 것인지에 대한 의사결정 구조를 포함해야 한다고 생각합니다. 
이러한 기준을 명확히 정의하고 통제하는 것은 언론 시스템의 역할이자 중요한 책임이라고 생각하여 이러한 설계를 채택하게 되었습니다.
